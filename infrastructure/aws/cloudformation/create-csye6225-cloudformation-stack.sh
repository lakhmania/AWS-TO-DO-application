...
export a=$(aws rds describe-db-instances --query "DBInstances[].DBInstanceIdentifier" --output text)

aws rds modify-db-instance --db-instance-identifier $a --enable-iam-database-authentication --apply-immediately

export rds_instance_id=$(aws rds describe-db-instances --query "DBInstances[].Endpoint.Address" --output text)

echo rds_instance_id $rds_instance_id

mysql -h $rds_instance_id -P 3306 -u csye6225master -p

CREATE USER csye6225-user IDENTIFIED WITH AWSAuthenticationPlugin as 'RDS';
